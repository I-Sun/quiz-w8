...
dataset_dir=/data/insun/udacity-2 # 数据集目录，这里是写死的，记得修改
...
config=ssd_mobilenet_v1_udacity.config
...
for i in {56..60}  # for循环中的代码执行5此，这里的左右边界都包含，也就是一共训练500个step，每100step验证一次
do
    echo "############" $i "runnning #################"
    last=$[$i*500]
    current=$[($i+1)*500]
